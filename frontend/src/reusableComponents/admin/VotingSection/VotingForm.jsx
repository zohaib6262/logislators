...
import { initialVotingSection } from "@/hooks/VotingSection/useVotingSection";

const VotingForm = ({ votingCard, onSubmit, onCancel }) => {
  const [formData, setFormData] = useState(initialVotingSection);
  ...
  useEffect(() => {
    if (votingCard) {
      setFormData(votingCard);
    } else {
      setFormData(initialVotingSection);
    }
    setErrors({});
    setTouched({});
  }, [votingCard]);
